    ...
    static func albumsWithJSON(results: NSArray) -> [Album] {
        ...
        if results.count>0 {
            ...
            for item in results {
                ...
                if let collectioId = result["collectionId"] as? Int{
                    var newAlbum = Album(name: name!, price: price!, thumbnailImageURL: thumbnailURL, largeImageURL: imageURL, itemURL: itemURL!, artistURL: artistURL, collectionId: collectioId)
